...
public partial class Builder_EditBuilder : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        cmd.CommandText = "Update Builder " +
                            "set BuilderName=@buildername,CompanyName=@companyname,CompanyLogo=@companylogo,OperatingSince=@operatingsince,Title=@title,TransactionType=@transactiontypes,PropertyType=@propertytypes,DetailDescription=@detaildescription,CurrentProjects=@currentprojects, PastProjects=@pastprojects, City=@city,Locality=@locality " +
                            "where userid=@userid";
        ...
        if (result > 0)
        {
            multiView.SetActiveView(viewBuilderUpdated);
        }
    }
}